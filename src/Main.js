...
const steps = ['The backstory', "What kind of job do you want?", "Results"];
...
export default function Main() {
  ...
  async function getJobs () {
    const encodedJobTitle = encodeURIComponent(state.jobTitle);
    const encodedJobLocation = encodeURIComponent(state.jobLocation);
    try {
      const resp = await fetch(
        `https://api.zacharyjklein.com/resumayday/?job_title=${encodedJobTitle}&location=${encodedJobLocation}`
      );
      return resp.json()
    } catch (err) {
      return []
  ...
  function getStepContent(step, state) {
    switch (step) {
      ...
      case 2:
        return <AppResults state={state} updater={setState}/>;
      default:
